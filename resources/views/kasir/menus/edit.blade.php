<x-admin-layout>
    <x-slot name="navbar">
        <x-tailadmin.navbar :breadcrumbs="[['label' => 'menu', 'url' => route('menus.index')], ['label' => 'edit']]" />
    </x-slot>
            ...
            <form method="POST" action="{{ route('menus.update', $menu->id) }}" enctype="multipart/form-data">
                ...
                <div class="-mx-2.5 flex flex-wrap gap-y-5">
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <input type="number" name="harga" placeholder="Masukkan harga" value="{{ $menu->harga }}"
                            class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                            required>
                        @error('harga')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <textarea name="deskripsi" placeholder="Masukan deskripsi (optional)" type="text" rows="6"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ $menu->deskripsi }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="w-full px-2.5">
                        <div class="flex items-center gap-3 mt-1">
                            ...
                            <a href="{{ route('menus.index') }}"
                                class="inline-block gap-2 rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                                Cancel
                            </a>
                        </div>
                    </div>
